...
static const int kFieldCounts[] = {0, 2, 15, 4, 13};
...
int gp_encode(const GpMessage *m, char *buf, size_t bufSize) {
  ...
  switch (m->type) {
    ...
    case GP_CMD:
      ...
      break;
    case GP_TUNE:
      n = snprintf(payload, sizeof(payload), "TUNE,%.2f,%.2f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.2f,%.2f,%.1f,%.1f",
                   ...
                   (double)m->tune.steamGapS, (double)m->tune.steamMinTemp, (double)m->tune.pumpFlow);
      break;
    default:
      buf[0] = '\0';
      return -1;
...
GpResult gp_decode(const char *line, size_t len, GpMessage *out) {
  ...
  switch (type) {
    ...
    case GP_TUNE:
      ...
      out->tune.pumpFlow = toFloat(f[12]);
      break;
    default:
      return GP_ERR_TYPE;
  }
