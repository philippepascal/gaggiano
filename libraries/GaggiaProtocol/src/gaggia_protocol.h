// Gaggiano screen/controller protocol v6. See docs/PROTOCOL.md.
// Plain C++ (no Arduino, no heap) so both firmwares and the host tests share it.
#pragma once
#include <stddef.h>
#include <stdint.h>

#define GP_PROTOCOL_VERSION 6
...
struct GpTune {
  ...
  float steamMinTemp; // v5: steam assist, no shot below this boiler temperature (C)
  float pumpFlow;     // v6: pump flow at full range at brew pressure (ml/s), brew heater feed-forward
};
