...
void linkSendTune() {
  ...
  m.tune.pumpFlow = adv->pump_flow_ml_s;
  sendMessage(m);
  logEvent("tune bb=%.2f cycle=%.0f kp=%.3f ki=%.3f kd=%.3f step=%.3f pkp=%.3f pki=%.3f pkd=%.3f shot=%.2f gap=%.2f min=%.1f flow=%.1f",
           ...
           (double)state->steam_gap_s, (double)state->steam_min_temp, adv->pump_flow_ml_s);
}
