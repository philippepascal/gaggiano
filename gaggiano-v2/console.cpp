...
static void printStatus(uint32_t now) {
  ...
  Serial.printf("STATUS profile=%s heat=%d brew=%d steam=%d clean=%d bloom=%d auto=%d phase=%d "
                "controller=%s ctrlMode=%d ctrlLinkOk=%d temp=%.2f press=%.2f valve=%d rx=%lu rxRejected=%lu rxOverflows=%lu tx=%lu "
                ...
                linkControllerMode(), state.linkOk, state.tempRead, state.pressureRead, state.isSolenoidOn,
